...
def get_udp(validation_mode,n):
    if not validation_mode:
        return B3_UDP(getLowerBound(),getUpperBound())
    else:
        raise RuntimeError('No validation')

class B3Terminator(TerminatorBase):
    def should_stop(self, pg_island, monitor):
        ...
        if best_f is None:
            ...
            return mean(best_f) < 0.02

sbmlfile = join('..','..','..','..','..','sbml','b3.xml')
...
config = BiopredynConfiguration.from_cmdline_args('b3-driver', get_udp, getDefaultParamValues, sbmlfile=sbmlfile, spark_files=spark_files, py_files=py_files, terminator=B3Terminator())
